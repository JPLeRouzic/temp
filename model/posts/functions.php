...
function get_post_unsorted($globpath): array {
    $sorted1 = array();

    // content/users/admin/blog/english/post
    // files unsorted
    $sorted1 = glob($globpath, GLOB_NOSORT);
...
function get_posts_sorted(string $globpath): array {
    // content/users/admin/blog/english/post
    $sorted1 = glob($globpath, GLOB_NOSORT);
// echo '<br>globpath: ' . $globpath ;
// echo '<br>nb posts: ' . count($sorted1) ;
// die() ;
...
// Find current post by name.
// Return an array of posts (previous, current, next)
function find_post_name(string $name): array|null {
    // content/users/admin/blog/english/post/$name.md
    $globpath = 'content/users/*/blog/*/post/*' . $name . '.md' ;
...
// Find current post by year, month and name.
// Return an array of posts (previous, current, next)
function find_posts(string $year, string $month, string $name): array|null {
    // content/users/admin/blog/english/post/$name.md
    $globpath = 'content/users/*/blog/*/post/' . $year . '-' . $month . '-*' . $name . '.md' ;
...
function duplicate_code_8(array &$message, string &$oldfile, string &$title, string &$image,
    ...
    if ($proper && !empty($title1) && !empty($tag1) && !empty($content) && !empty($image) && ($image !== '')) {
        if (empty($url)) {
            $url = $title->value;
        }
