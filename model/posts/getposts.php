...
function get_posts(array|null $posts1, int $page = 1, int $perpage = 0): array {
    if (empty($posts1)) {
        ...
        $posts1 = get_posts_sorted($globpath);
    }
...
// string(39) "content/users/system/blog/english/post/"
// string(50) "content/users/admin/blog/中文/post/2021-08-16-18-49-20"
...
/* */        
   $min = min(count($t), count($tl));
    $tCom = array_combine(array_slice($t, 0, $min), array_slice($tl, 0, $min));
...
// Related posts (related by same tags)
    ...
    /*
     * Extract the path to main picture
     */
    $post->img_url = get_picture_URL($post->body);

    return $post;
}

/*
 * Returns the URL (or file path) of the first (main) image in post
 */

function get_picture_URL(string $post): string {
    $img1 = explode('<img src="', $post);
    if ((!is_array($img1)) || (count($img1) < 2)) {
        return site_url() . "assets/PI_logo_square.avif";
    }

    $img2 = explode('"', $img1[1]);

    return $img2[0]; // The URL of picture
}
