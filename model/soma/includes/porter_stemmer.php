...
/**
 * Copyright (c) 2005 Richard Heyes (https://www.phpguru.org/)
 *
 * All rights reserved.
 *
...
class PorterStemmer {
     ...
     * Stems a word.
     *
     * @param  string $word6 Word to stem
     * @return string       Stemmed word
     */
    public static function Stem($word6) {
        if (strlen($word6) <= 2) {
            return $word6;
        }

        $word5 = self::step1ab($word6);
        $word4 = self::step1c($word5);
        $word3 = self::step2($word4);
        $word2 = self::step3($word3);
        $word1 = self::step4($word2);
        $word = self::step5($word1);
     ...
     * <c>vcvcvc<v> gives 3
     *
     * @param  string $str2 The string to return the m count for
     * @return int         The m count
     */
    private static function m($str2) {
        $c = self::$regex_consonant;
        $v = self::$regex_vowel;

        $str1 = preg_replace("#^$c+#", '', $str2);
        $str = preg_replace("#$v+$#", '', $str1);
